<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-3">
            ...
            <div>
                <p class="text-xs font-semibold uppercase tracking-[0.2em] text-orange-500">Pedidos</p>
                <h2 class="text-2xl font-bold text-gray-900" style="font-family:'Playfair Display',serif;">Editar Pedido</h2>
            </div>
        </div>
    </x-slot>
    ...
    <style>
        ...
        .field-input:focus{border-color:#f97316;background:#fff;box-shadow:0 0 0 4px rgba(249,115,22,.08);}
        ...
        .field-group:focus-within .field-icon{color:#f97316;}
    ...
    <div class="py-10 dm">
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="form-wrap flex items-center gap-3 px-1 mb-4">
                ...
                <div>
                    <p class="text-sm font-semibold text-gray-800 font-mono">{{ $pedido->numero_pedido }}</p>
                    <p class="text-xs text-gray-400">Editando pedido</p>
                </div>
            </div>
                    ...
                    <form action="{{ route('pedidos.update', $pedido->id) }}" method="POST" class="space-y-5">
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                ...
                                <div class="field-group">
                                    ...
                                    <input type="text" name="numero_pedido" value="{{ old('numero_pedido', $pedido->numero_pedido) }}" class="field-input" required>
                                </div>
                            ...
                            <div>
                                ...
                                <div class="field-group">
                                    ...
                                    <input type="date" name="data_pedido" value="{{ old('data_pedido', \Carbon\Carbon::parse($pedido->data_pedido)->format('Y-m-d')) }}" class="field-input" required>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                ...
                                <div class="field-group">
                                    ...
                                    <input type="number" name="valor_total" value="{{ old('valor_total', $pedido->valor_total) }}" class="field-input" step="0.01" min="0" required>
                                </div>
                            </div>
                            <div>
                                ...
                                <div class="field-group">
                                    ...
                                    <select name="status" class="field-input" required>
                                        @php $cur = old('status', $pedido->status); @endphp
                                        <option value="pendente"    {{ $cur === 'pendente'    ? 'selected' : '' }}>Pendente</option>
                                        <option value="processando" {{ $cur === 'processando' ? 'selected' : '' }}>Processando</option>
                                        <option value="concluído"   {{ $cur === 'concluído'   ? 'selected' : '' }}>Concluído</option>
                                        <option value="cancelado"   {{ $cur === 'cancelado'   ? 'selected' : '' }}>Cancelado</option>
                                    </select>
                                </div>
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <div class="field-group">
                                ...
                                <textarea name="observacoes" class="field-input" rows="3">{{ old('observacoes', $pedido->observacoes) }}</textarea>
                            </div>
                        </div>

                        <div class="flex items-center justify-between pt-2">
                            <a href="{{ route('pedidos.index') }}" class="text-sm text-gray-400 hover:text-gray-600 font-medium transition">← Cancelar</a>
                            <button type="submit" class="inline-flex items-center gap-2 px-6 py-2.5 bg-orange-500 text-white text-sm font-semibold rounded-full hover:bg-orange-600 transition-all duration-300 shadow-md hover:shadow-orange-200 active:scale-95">
