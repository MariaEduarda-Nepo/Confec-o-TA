<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <div>
                <p class="text-xs font-semibold uppercase tracking-[0.2em] text-blue-500 mb-1">Gestão</p>
                <h2 class="text-2xl font-bold text-gray-900 leading-tight" style="font-family: 'Playfair Display', serif;">
                    Fornecedores
                </h2>
            </div>
            <a href="{{ route('fornecedores.create') }}" class="group inline-flex items-center gap-2 px-5 py-2.5 bg-gray-900 text-white text-xs font-semibold uppercase tracking-widest rounded-full hover:bg-blue-600 transition-all duration-300 shadow-md hover:shadow-blue-200">
    ...
    <div class="py-10 dm">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-5">
            ...
            <div class="flex items-center gap-3">
                <span class="text-sm text-gray-400 font-medium">{{ $fornecedores->count() }} {{ $fornecedores->count() === 1 ? 'fornecedor' : 'fornecedores' }}</span>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                @forelse ($fornecedores as $fornecedor)
                    <div class="card-item group bg-white border border-gray-100 rounded-3xl p-6 hover:shadow-xl hover:-translate-y-1 transition-all duration-300 relative overflow-hidden">
                        <div class="absolute top-0 left-6 right-6 h-0.5 bg-gradient-to-r from-blue-400 to-indigo-400 rounded-full opacity-0 group-hover:opacity-100 transition-opacity duration-300"></div>

                        <div class="flex items-start gap-4 mb-5">
                            <div class="w-12 h-12 bg-blue-600 rounded-2xl flex items-center justify-center flex-shrink-0">
                                <span class="text-white font-bold text-base" style="font-family:'Playfair Display',serif;">{{ strtoupper(substr($fornecedor->nome, 0, 1)) }}</span>
                            </div>
                            <div class="min-w-0">
                                <h3 class="font-semibold text-gray-900 truncate">{{ $fornecedor->nome }}</h3>
                                <p class="text-xs text-gray-400 font-mono mt-0.5">{{ $fornecedor->cnpj }}</p>
                            </div>
                        </div>
                        ...
                        <div class="space-y-2.5">
                            <div class="flex items-center gap-2.5 text-sm">
                                ...
                                <span class="text-gray-600 font-medium">{{ $fornecedor->telefone }}</span>
                            </div>
                            <div class="flex items-center gap-2.5 text-sm">
                                ...
                                <span class="text-gray-600 truncate text-xs">{{ $fornecedor->email }}</span>
                            </div>
                            <div class="flex items-start gap-2.5 text-sm">
                                ...
                                <span class="text-gray-500 text-xs leading-relaxed">{{ $fornecedor->endereco }}, {{ $fornecedor->cidade }} – {{ $fornecedor->estado }}</span>
                            </div>
                        </div>

                        <div class="flex items-center justify-end mt-5 pt-4 border-t border-gray-100 gap-2">
                            <a href="{{ route('fornecedores.edit', $fornecedor->id) }}" class="inline-flex items-center gap-1.5 px-3.5 py-1.5 text-xs font-semibold text-gray-600 bg-gray-100 hover:bg-blue-100 hover:text-blue-700 rounded-full transition-all duration-200">
                            ...
                            <form action="{{ route('fornecedores.destroy', $fornecedor->id) }}" method="POST" onsubmit="return confirm('Remover {{ $fornecedor->nome }}?')">
                                @csrf @method('DELETE')
                                <button type="submit" class="inline-flex items-center gap-1.5 px-3.5 py-1.5 text-xs font-semibold text-gray-400 hover:bg-red-50 hover:text-red-600 rounded-full transition-all duration-200">
                ...
                @empty
                    <div class="col-span-full py-20 flex flex-col items-center">
                        ...
                        <p class="text-gray-400 font-medium">Nenhum fornecedor cadastrado</p>
                    </div>
                @endforelse
            </div>
        </div>
